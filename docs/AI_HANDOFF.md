## Animações do onboarding sem loop — 2026-09-10

- Em andamento na branch `fix/onboarding-animations-once`, criada de `origin/main` em `6141e0b`, no checkout principal.
- Objetivo: as ilustrações do bottom sheet `Cómo funciona Draftea Pulse` rodavam em loop infinito. Pedido da pessoa usuária: rodar uma vez só e parar no quadro final.
- Implementação: as keyframes não mudaram. Cada card ganhou uma parada fracionária somada a `animation-fill-mode: forwards` — `--onboarding-chart-hold: 0.88`, `--onboarding-shares-hold: 0.92`, `--onboarding-series-hold: 0.88` e, no card 2, `--onboarding-clock-hold: 1.88` com `--onboarding-clock-cycle-hold` sendo a metade disso, para as duas escalas de tempo pararem no mesmo instante. A fração escolhida é o ponto em que o quadro cheio já está montado, logo antes da dissolvência que só existia para emendar o loop.
- Card 2, depois de dois pedidos da pessoa usuária na mesma sessão: ficou só a volta que fecha acima do objetivo, e a barra de progresso deixou de sair de cena — ela enche, permanece cheia e então a pílula `Terminó arriba` aparece. Saíram a segunda volta, o wrapper `__runs`, as classes `__run--a`/`__run--b`, as keyframes das duas, o modificador `__outcome--down` e as variáveis `--onboarding-clock-cycle`/`--onboarding-clock-cycle-hold`; `--onboarding-clock-hold` voltou a `0.88`. Como a pílula não ocupa mais o lugar da barra, as duas passaram a dividir a faixa livre entre o cabeçalho (48px) e os preços (98px), flutuando uma sobre a outra: as duas conservam as posições do Figma — barra em `71px` e pílula em `64.77px`, centradas na mesma linha em 73px. A pílula pinta por cima por vir depois no DOM, sem `z-index`, e com 110px contra os 323px da barra o preenchimento aparece dos dois lados. Chegamos aqui em duas correções da pessoa usuária: primeiro as duas foram empilhadas, depois ela esclareceu que queria a pílula flutuando sobre a barra, e não acima dela.
- Compensação registrada: com uma volta só, a ilustração não mostra mais uma rodada fechando abaixo do objetivo. Era o que o comentário original do componente dava como motivo das duas voltas; a decisão de manter apenas `Terminó arriba` é da pessoa usuária.
- Card 4: `OnboardingSellEarly` deixou de remontar a série depois de 2s. A venda — automática em 48% da varredura ou antecipada pelo toque — congela a ilustração e o card fica nesse quadro. Saíram `SOLD_HOLD_MS`, o temporizador de reset, o estado `cycle`, a `key` da série e os refs `armed`/`previousPhase`, que existiam só para o rearme entre voltas.
- Swipe no carrossel, pedido depois e refeito duas vezes. A primeira tentativa foi um gesto próprio de ponteiro sobre o palco; ela falhou na prática e foi descartada. O diagnóstico com ponteiro real mostrou a causa: o arrasto que começa sobre uma imagem — a série do card 1 — vira o arrastar-e-soltar nativo do navegador e morre num `pointercancel` no primeiro movimento, com a sequência `pointerdown → pointermove → pointercancel` registrada no navegador. Junto disso apareceram mais três fraquezas: o texto era selecionado durante o arrasto, o eixo era decidido no primeiro movimento e um começo em diagonal matava o gesto inteiro, e a área do gesto era só o palco, não o corpo do sheet.
- Depois disso a pessoa usuária apontou o problema maior: mesmo funcionando, a troca por dissolvência com deslocamento de 24% não deixava perceber que o card tinha mudado, e pediu um slider de verdade, no modelo do `Últimas 10 rondas`, mantendo a estrutura do sheet e os bullets. O gesto próprio inteiro foi removido — ponteiro, roda, guarda de clique, `--onboarding-swipe-x`, `leavingStep`, `direction` e as quatro keyframes de entrada e saída — e deu lugar a uma trilha de rolagem nativa com encaixe, igual à do `PreviousRounds`: `scroll-snap-type: x mandatory`, slides de largura cheia com `scroll-snap-align: start`, passo ativo derivado da rolagem e `scrollTo` nos botões. Quem arrasta, encaixa e dá inércia passou a ser o navegador, o que resolve toque e trackpad de uma vez e elimina o código de gesto.
- Recuo lateral, apontado pela pessoa usuária depois: o arrasto acontecia dentro de uma janela recuada, porque os `16px` de `padding` do corpo do sheet limitavam a trilha. Eles desceram para a trilha (`padding-inline: 16px` com `scroll-padding-inline: 16px`, exatamente a composição do `PreviousRounds`) e para bullets e rodapé; o corpo ficou com `padding: 16px 0`. `scrollToStep` passou a medir a partir do primeiro slide, e não da trilha, para o recuo se cancelar na conta.
- Ilustrações: com os quatro slides coexistindo, montar tudo faria as animações — que agora rodam uma vez só — terminarem fora da tela. Só o passo ativo e o vizinho são montados, e o vizinho fica em `animation-play-state: paused` no quadro zero até chegar a vez, preso ao `data-active` do slide. Montar o vizinho é o que evita o card entrar vazio durante o arrasto. `user-select: none`, `-webkit-user-drag: none` e `overscroll-behavior-x: contain` permaneceram, porque as três causas que eles cobrem valem igual para a trilha.
- Validação local: `pnpm lint` limpo, `pnpm build` concluído (permanece o aviso conhecido de chunk acima de 500 kB), `pnpm test:maze` com 40 testes passando e `git diff --check` limpo.
- Validação no navegador a 375×812, em `?task=onboarding`: card 1 com as sete animações em `finished` aos 4576ms, série inteira, marcador na ponta, pílula e escolha UP acesas; card 3 em `finished` aos 7176ms, conta inteira e contador parado em `$14.93`; card 4 vendido sozinho aos 2517ms de 5200, `Vendido por: $177.37`, botão `Vendido` desabilitado e linha do momento em 309px, sem reinício depois de mais 6s. Voltar ao card 1 remontou e a animação recomeçou do zero. Console sem erros novos — permanecem os de rede das fontes de mercado, anteriores a esta mudança.
- Card 2 revalidado depois da reformulação: `finished` aos 4576ms, com 00:00, barra cheia e visível (`opacity: 1`, preenchimento igual à trilha em 323px), `Terminó arriba` em verde, `Precio final $80,202.64` e `↑ $7`. Caixas medidas dentro da área de 156px: cabeçalho `0 → 48`, barra `71 → 75` ocupando `x 32 → 355`, pílula `64.77 → 81.23` ocupando `x 146 → 241` — centrada sobre a barra e deixando 114px de preenchimento visível de cada lado —, preços `98 → 156`. Aos 2,6s, no meio da contagem, a barra já aparecia com o preenchimento parcial e a pílula ainda estava em `opacity: 0`.
- Slider validado no navegador: trilha com `scroll-snap-type: x mandatory`, `scrollWidth` 1604 para `clientWidth` 389, quatro slides de 389px em `offsetLeft` 16/421/826/1231 — passo de 405px. Levar a rolagem a 0/405/810/1215 moveu o passo ativo, os bullets (`X...` → `.X..` → `..X.` → `...X`), a presença do `Volver` e o CTA, que virou `Entendido, empezar` só no quarto. Uma posição intermediária (1100) foi encaixada em 1215 pelo próprio `scroll-snap`. Montagem e pausa conferidas slide a slide: o ativo `running`, o vizinho `paused` no quadro zero, os distantes sem ilustração montada; voltar a um passo já visitado remontou e a animação voltou a rodar.
- Geometria conferida depois da correção do recuo, num sheet de 421px: a trilha ocupa `0 → 421` e o slide em repouso `16 → 405`, com ilustração, título e corpo nas mesmas coordenadas de antes. Encaixado no segundo slide, o primeiro fica em `-389 → 0` — a borda direita dele alcança a borda esquerda do sheet, que era o que faltava. O CTA termina em `405`, os bullets seguem centrados e `document.scrollWidth` continua igual ao viewport.
- **Não validado nesta sessão**: o arrasto com o dedo e o gesto do trackpad em input real, e o tempo das animações. O painel de preview entrou em ~1 quadro por segundo (medido: 1 `requestAnimationFrame` em 1s, com `visibilityState: visible`), o que congela o scroll suave, a entrega dos eventos `scroll` do navegador e o relógio das animações; as ações de clique e arrasto do painel passaram a expirar em 30s. A validação acima foi feita despachando o evento `scroll` à mão sobre o mesmo handler. A causa é a janela do app estar coberta, não o código.
- Concluído: commit `be34a10`, Pull Request #27, merge `dfd2f60` e deploy `34529046010` com sucesso. O CSS publicado em `https://design-draftea.github.io/pulse-maze-study/` traz `onboarding-sheet__track`, `scroll-snap-type: x mandatory` e `scroll-padding-inline: 16px`, e nenhuma animação do onboarding conserva `infinite`.
- Pendente: conferir o arrasto com o dedo e o gesto do trackpad num dispositivo real sobre a versão publicada, que é o que não foi possível exercitar localmente.
...
# Correção da captura de conclusão — 2026-09-08

- Objetivo: eliminar falsos `Task unfinished` mantendo os critérios reais das quatro tarefas.
- Branch: `fix/maze-completion-capture`; alterações locais em `src/services/mazeStep.ts`, `tests/mazeAutoEnd.test.ts`, `tests/mazeCapture.browser.html` e este handoff.
- Diagnóstico: o script público `lwtScaffold.js` do Maze detecta mudanças de URL em um `MutationObserver` de `childList`. O encerramento síncrono anterior podia acontecer antes da entrega desse observer. Ausência de URL final no relatório não comprova que a pessoa deixou de executar a ação.
- Implementação: após `replaceState`, uma mutação neutra de comentário garante a observação sem alterar a interface. O clique no widget ocorre no próximo timer, com 250 ms para a entrega da navegação; não há refresh nem resultado forjado. Mantida proteção contra sobrescrita da conclusão.
- Maze: os quatro links do rascunho Draftea Pulse agora usam `index.html`; venda preserva `task=sell`, histórico preserva `task=tracking`. Os quatro paths foram regravados por ações reais e salvos, terminando em `onboarding-complete`, `purchase-complete`, `sale-complete` e `past-entries-open`. Texto e critérios de pesquisa preservados.
- Validação: 39 testes Maze aprovados, lint aprovado, build aprovado (aviso preexistente de tamanho de chunk). Harness no Chrome confirmou os quatro finais com MutationObserver real e widget simulado: URL observada antes do clique. Isso não comprova a classificação no relatório real.
- Pendente: autorização solicitada para PR, merge/deploy e publicação do estudo. Nenhum desses passos foi realizado nesta sessão. Depois da publicação do protótipo, validar as quatro tarefas em QA isolado e conferir os paths registrados e `Task completed`; não contaminar respostas reais nem reclassificar falhas para esconder o problema.
- Servidor local da regressão: porta 5196, página `/tests/mazeCapture.browser.html` (não incluída no build de produção).
