# Plano da pesquisa — Draftea Pulse no Maze

Documentação para revisão interna. **Não traduzir ainda**: a tradução para o
espanhol do México acontece depois da aprovação deste conteúdo. A interface do
protótipo já está em espanhol; apenas este plano e as perguntas do Maze estão em
português nesta etapa.

## Objetivo

Entender se usuários da Draftea conseguem compreender e utilizar o Pulse sem
acompanhamento de um moderador.

## Estrutura

1. Conhecer o Pulse pelo onboarding.
2. Fazer uma entrada de US$10.
3. Vender a entrada antes do fim da rodada.
4. Encontrar uma resposta no Pergunte ao Pulse.
5. Responder perguntas curtas após cada tarefa.
6. Preencher uma avaliação final.

## Mensagem inicial

**Título**

Ajude a melhorar o Draftea Pulse

**Texto**

Nesta atividade, você vai conhecer uma nova experiência da Draftea e realizar
algumas tarefas.

Use o Pulse como usaria normalmente. Durante o teste, responda com base no que
você entendeu e sentiu.

A atividade dura aproximadamente 10 minutos. Todos os valores e operações são
simulados.

## Consentimento

Durante o teste, podemos registrar sua interação com a experiência para entender
como ela é utilizada. Os dados serão usados apenas para analisar e melhorar o
produto.

Você aceita participar?

1. Sim, aceito.
2. Prefiro não participar.

## Perguntas iniciais

**Pergunta 1**

Antes de hoje, você já havia utilizado algum produto em que precisava prever se
o preço de um ativo subiria ou cairia?

1. Sim, várias vezes.
2. Sim, uma ou duas vezes.
3. Nunca.
4. Não tenho certeza.

**Pergunta 2**

Com que frequência você acompanha o preço do Bitcoin ou de outras criptomoedas?

1. Todos os dias.
2. Algumas vezes por semana.
3. Algumas vezes por mês.
4. Quase nunca.
5. Nunca.

## Tarefa 1: onboarding

**Texto da tarefa**

É a sua primeira vez no Draftea Pulse. Antes de fazer uma operação, descubra como
ele funciona.

Finalize a tarefa quando sentir que já entendeu o que precisa fazer.

**Pergunta aberta**

Com suas próprias palavras, como funciona uma rodada no Draftea Pulse?

**Pergunta de múltipla escolha**

No exemplo apresentado, o que significa UP estar em 67%?

1. Cada participação custa US$0,67.
2. O Bitcoin precisa subir 67%.
3. Vou receber 67% do valor utilizado.
4. Meu ganho será de US$0,67.
5. Não tenho certeza.

**Escala**

Quanto o guia ajudou você a entender como o Pulse funciona?

1 significa “não ajudou”.
7 significa “ajudou muito”.

## Tarefa 2: compra

**Texto da tarefa**

Imagine que você acredita que o preço do Bitcoin terminará acima do preço
objetivo.

Use US$10 nessa previsão e confirme a operação.

**Escala**

Quão fácil ou difícil foi fazer a entrada?

1 significa “muito difícil”.
7 significa “muito fácil”.

**Pergunta de múltipla escolha**

Ao usar US$10, a tela mostrou um valor em “Ganancia potencial”. O que esse valor
representa?

1. O valor total que eu receberia.
2. Meu ganho, sem contar os US$10 utilizados.
3. O valor que usei para fazer a entrada.
4. O preço de uma participação.
5. Não tenho certeza.

**Pergunta opcional de múltipla escolha**

Esse valor em “Ganancia potencial” inclui os US$10 que você usou?

1. Sim, é o total que eu receberia: os US$10 mais o ganho.
2. Não, é só o ganho, que viria além dos US$10.
3. Não tenho certeza.

## Tarefa 3: venda

**Texto da tarefa**

Depois de fazer a entrada, você decide sair antes que a rodada termine.

Venda toda a sua entrada.

**Escala**

Quão fácil ou difícil foi vender sua entrada?

1 significa “muito difícil”.
7 significa “muito fácil”.

**Pergunta de múltipla escolha**

O que determina quanto você recebe ao vender antes do final da rodada?

1. O valor das minhas participações naquele momento.
2. O valor que usei na compra.
3. O preço objetivo da rodada.
4. Cada participação sempre paga US$1.
5. Não tenho certeza.

**Pergunta aberta opcional**

Alguma informação deixou você com dúvida durante a venda?

## Tarefa 4: ajuda

**Texto da tarefa**

Você percebeu que o preço do Bitcoin mostrado no Pulse está um pouco diferente do
preço visto em outra plataforma.

Use o Pergunte ao Pulse para descobrir por que isso pode acontecer.

**Pergunta de múltipla escolha**

A resposta do Pergunte ao Pulse resolveu sua dúvida?

1. Sim, completamente.
2. Sim, parcialmente.
3. Não.
4. Não encontrei uma resposta.

**Escala**

Quão confiável pareceu a resposta apresentada?

1 significa “nada confiável”.
7 significa “muito confiável”.

**Pergunta aberta opcional**

Que outra pergunta você faria ao Pulse?

## Avaliação final

Use uma escala com:

1. Discordo totalmente.
2. Discordo.
3. Nem concordo nem discordo.
4. Concordo.
5. Concordo totalmente.

Afirmações:

1. Entendi o que precisava prever em uma rodada.
2. Entendi o que é o preço objetivo.
3. Entendi o que representa o percentual de UP ou DOWN.
4. Ficou claro quanto eu poderia receber caso acertasse.
5. Consegui diferenciar o valor total recebido do meu ganho.
6. Entendi como vender antes do final da rodada.
7. As informações de preço pareceram claras.
8. Eu me sentiria seguro usando essa experiência.

## Perguntas finais

**Obrigatória**

O que foi mais confuso para você durante o teste?

**Opcional**

Caso pudesse mudar uma coisa no Draftea Pulse, o que mudaria primeiro?

**Escala de interesse**

Qual seria o seu interesse em usar o Draftea Pulse caso ele estivesse disponível?

1 significa “nenhum interesse”.
5 significa “muito interesse”.

**Opcional**

Existe mais alguma coisa que você gostaria de comentar sobre a experiência?

## Encerramento

**Título**

Obrigado por participar!

**Texto**

Suas respostas vão nos ajudar a tornar o Draftea Pulse mais claro, simples e
fácil de usar.

Agradecemos pelo seu tempo e pela contribuição.

---

## Nota de revisão: por que as perguntas não citam valores

O mercado do protótipo se move. O preço do Bitcoin caminha durante a missão e
UP e DOWN acompanham, em degraus de um ponto percentual, como no produto real.
A caminhada é determinística — é função do tempo decorrido desde a abertura da
tarefa, então é o mesmo filme para todo participante —, mas cada pessoa age num
instante diferente e portanto compra a um preço diferente.

Na prática: quem confirma a compra com UP em 67% recebe 14,93 participações e vê
US$14,93 em “Ganancia potencial”; quem confirma com UP em 65% recebe 15,38 e vê
US$15,38. **Por isso nenhuma pergunta pode citar um valor.**

O que continua fixo e pode ser citado:

- O **monto usado** é sempre US$10, então o saldo depois da compra é sempre
  US$2.030,00.
- O **exemplo do onboarding** é ilustração estática, não dado de mercado: o card
  do terceiro passo mostra sempre US$10, 67¢, 14,93 participações, US$14,93 e
  +US$4,93. As perguntas da tarefa 1 se apoiam nele com segurança.
- A **abertura de qualquer tarefa** mostra sempre UP em 67% e DOWN em 33%. O que
  varia é o que acontece depois.

O que se perde: não dá mais para comparar entre participantes se alguém “acertou
US$14,93”. A compreensão passa a ser medida pela relação entre os números — se a
pessoa entende que o valor mostrado inclui o monto usado —, e não pelo número em
si. É por isso que a pergunta opcional virou uma pergunta de inclusão, e não de
aritmética.

O que se ganha: a tarefa 3 fica melhor sustentada. A pergunta “o que determina
quanto você recebe ao vender antes do final da rodada” tem como resposta correta
“o valor das minhas participações naquele momento” — e agora esse valor
visivelmente muda na tela enquanto a pessoa decide, em vez de ficar parado
contradizendo a alternativa certa.

Permanece em aberto um ponto de conteúdo do produto: o betslip mostra um único
número sob o rótulo `Ganancia potencial`, e esse número é o total recebido, não o
ganho. Os dois só aparecem separados no onboarding. A tela foi mantida como está
porque o rótulo é conteúdo aprovado e a ambiguidade é justamente o que a pergunta
mede — corrigi-la antes da coleta responderia a pergunta no lugar do
participante. Na análise, a alternativa 2 é a leitura que o rótulo induz e a 1 é
a correta; uma taxa alta na 2 é um resultado acionável sobre o rótulo, não
desatenção do participante.
